...
- [x] Acceptance: `init()` creates `.search-hub/` structure in target directory with project-scoped config
...
- [x] Refactor if needed
- [x] Verify test still passes
- [x] Acceptance: `init --global` creates global config template with credential hints
...
- [x] Refactor if needed
- [x] Verify test still passes
- [x] Acceptance: CLI output provides actionable next-step hints

### Step 4: Update CLI command registration

- [x] Write test: `src/cli/index.ts` related tests
...
- [x] Refactor if needed
- [x] Verify test still passes
- [x] Acceptance: CLI correctly dispatches to local/global init based on flags

### Final Step: E2E Integration Tests (MANDATORY)

- [x] Write E2E test: `src/cli/commands/init.e2e.test.ts`
...
- [x] **Manual verification**: Run `search-hub init` in a real directory
